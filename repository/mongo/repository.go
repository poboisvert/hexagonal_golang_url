...
func (r *mongoRepository) Find(code string) (*shortener.Redirect, error) {
	...
	defer cancel()
	redirect := &shortener.Redirect{} // will be populate
	collection := r.client.Database(r.database).Collection("redirects")
	filter := bson.M{"key code is ": code}
	err := collection.FindOne(ctx, filter).Decode(&redirect) // call collection
	if err != nil {
	...
	return redirect, nil // redirect URL
}
